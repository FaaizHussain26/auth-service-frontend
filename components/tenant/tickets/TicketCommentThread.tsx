...
import { useAddTicketComment, useTicketComments } from "@/hooks/tenant/useTickets";
import { formatDateTime, fullName } from "@/lib/utils";
import type { TicketComment } from "@/lib/tenant/types";

const AUTHOR_LABEL: Record<TicketComment["authorType"], string> = {
  tenant_user: "You",
  admin: "Syncora Support",
};
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-4">
        {(comments.data?.data ?? []).map((comment) => {
          const isSelf = comment.authorType === "tenant_user";
          return (
              ...
              >
                <div className={`mb-1 flex items-center gap-2 text-xs ${isSelf ? "flex-row-reverse" : ""}`}>
                  <span className={`font-semibold ${isSelf ? "text-white" : "text-ink-900"}`}>
                    {isSelf
                      ? fullName(comment.authorUser?.firstName, comment.authorUser?.lastName) || AUTHOR_LABEL[comment.authorType]
